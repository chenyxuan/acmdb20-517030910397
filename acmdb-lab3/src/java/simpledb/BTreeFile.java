...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (pid.pgcateg() == BTreePageId.LEAF) {
			...
			if (f == null) {
				nextPageId = it.next().getLeftChild();
			} else {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreePageId oldRightPid = page.getRightSiblingId();

		rightPage.setRightSiblingId(oldRightPid);
		rightPage.setLeftSiblingId(page.getId());
		page.setRightSiblingId(rightPage.getId());
		if(oldRightPid != null) {
			BTreeLeafPage oldRightPage = (BTreeLeafPage) getPage(tid, dirtypages, oldRightPid, Permissions.READ_WRITE);
			oldRightPage.setLeftSiblingId(rightPage.getId());
			dirtypages.put(oldRightPid, oldRightPage);
		}

		Field middleKey = it.next().getField(keyField);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numEntriesToMove = (page.getNumEntries() - 1) / 2;
		Iterator<BTreeEntry> it = page.reverseIterator();
		while(numEntriesToMove > 0) {
			BTreeEntry entry = it.next();
		...
		middleEntry.setRightChild(rightPage.getId());

		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middleEntry.getKey());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		BTreeLeafPage leftPage = isRightSibling ? page : sibling;
		Tuple middleTuple = leftPage.reverseIterator().next();
		entry.setKey(middleTuple.getField(keyField));
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numEntriesToMove = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		while (numEntriesToMove > 0) {
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numTuplesToMove = rightPage.getNumTuples();
		while (numTuplesToMove > 0) {
			Tuple tuple = rightPage.iterator().next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry leftEntry = leftPage.reverseIterator().next();
		BTreeEntry rightEntry = rightPage.iterator().next();
		leftPage.insertEntry(new BTreeEntry(
				parentEntry.getKey(),
				leftEntry.getRightChild(),
				rightEntry.getLeftChild()));

		int numEntriesToMove = rightPage.getNumEntries();
		while(numEntriesToMove > 0) {
			rightEntry = rightPage.iterator().next();
